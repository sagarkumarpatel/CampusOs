...
export default function ProfilePage() {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto overflow-y-auto space-y-6">
      <div>
        <h1 className="text-3xl font-extrabold tracking-tight text-foreground">Profile Setup</h1>
        <p className="text-text-muted text-sm mt-1">Configure your personal and academic credentials for matching mentors.</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-surface border border-border backdrop-blur-xl p-8 rounded-3xl">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="text"
              {...register('firstName')}
              className="w-full px-4 py-3 rounded-xl bg-surface border border-border text-foreground placeholder-slate-500 focus:outline-none focus:border-accent-coral/50 transition-colors text-sm"
            />
        ...
        <div>
          <label className="block text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Graduation Year</label>
          <input
            type="number"
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-4 py-3 rounded-xl bg-surface border border-border text-foreground placeholder-slate-500 focus:outline-none focus:border-accent-coral/50 transition-colors text-sm resize-none"
          />
        </div>

        <div>
          <label className="block text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Skills (Comma-separated)</label>
          <input
            type="text"
        ...
        <button
          type="submit"
          disabled={loading}
          className="px-6 py-3 rounded-xl font-medium bg-gradient-to-r from-accent-coral to-accent-coral hover:from-accent-coral hover:to-orange-600 text-foreground shadow-lg shadow-accent-coral/20 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:scale-100 transition-all duration-200 text-sm"
        >
