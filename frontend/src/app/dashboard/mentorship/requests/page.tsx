...
export default function MentorshipRequestsPage() {
  ...
  return (
    <div className="p-8 space-y-8 max-w-4xl mx-auto overflow-y-auto">
      ...
      <div>
        <h1 className="text-3xl font-extrabold text-white tracking-tight">My Mentorship Requests</h1>
        <p className="text-slate-400 text-sm mt-1">Track the status of requests you've sent and received.</p>
      </div>
      ...
      {user?.role === 'MENTOR' && (
        <section className="space-y-4">
          ...
          {requests?.received?.length === 0 ? (
            <p className="text-slate-500 text-sm py-4">No one has sent you a mentorship request yet.</p>
          ) : (
                ...
                <div key={req.id} className="rounded-2xl bg-white/5 border border-white/10 overflow-hidden">
                  ...
                  <div className="p-5 flex flex-col md:flex-row justify-between gap-5">
                    <div className="space-y-3 flex-1">
                      ...
                      {req.studentBio && (
                        <p className="text-xs text-slate-400 line-clamp-2">{req.studentBio}</p>
                      )}

                      {req.studentSkills.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {req.studentSkills.slice(0, 5).map((s) => (
                            <span key={s} className="px-2 py-0.5 rounded text-[10px] bg-white/5 border border-white/8 text-slate-300">{s}</span>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="bg-black/30 border border-white/5 p-4 rounded-xl">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Their message</p>
                    ...
                    {req.status === 'PENDING' && (
                      <div className="flex md:flex-col gap-2 items-start shrink-0">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all disabled:opacity-50"
                        >
                        ...
                        <button
                          ...
                          disabled={mutation.isPending}
                          className="flex items-center gap-1.5 px-5 py-2.5 rounded-xl bg-white/5 hover:bg-rose-600 hover:text-white border border-white/10 text-slate-300 text-xs font-bold transition-all disabled:opacity-50"
                        >
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3 border-b border-white/10 pb-3">
          ...
          <h2 className="text-lg font-bold text-white">Requests I Sent</h2>
        </div>

        {requests?.sent?.length === 0 ? (
          <div className="text-center py-10 space-y-3">
            <p className="text-slate-500 text-sm">You haven't sent any mentorship requests yet.</p>
            <Link href="/dashboard/mentorship" className="inline-flex items-center gap-1.5 text-xs text-violet-400 hover:text-violet-300 font-semibold transition-colors">
              Browse Mentors →
            </Link>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="p-5 space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <h3 className="text-base font-bold text-white">{req.mentorName}</h3>
                        <p className="text-xs text-slate-400">{req.mentorTitle} @ <span className="font-semibold text-violet-400">{req.mentorCompany}</span></p>
                    ...
                    <div className="bg-black/20 border border-white/5 p-4 rounded-xl">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Your message</p>
                      <p className="text-sm text-slate-300 leading-relaxed whitespace-pre-wrap">{req.message}</p>
                    </div>
                    ...
                    {isAccepted && (
                      <div className="rounded-xl bg-emerald-500/5 border border-emerald-500/20 p-4 space-y-3">
                        ...
                        <p className="text-xs text-slate-300">
                          Your mentorship session request has been approved. Reach out to <span className="font-semibold text-white">{req.mentorName}</span> via LinkedIn to schedule your session.
                        </p>
                        <div className="flex flex-wrap gap-3 pt-1">
                          {req.linkedinUrl ? (
                            <a
                              ...
                              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs transition-all"
                            >
                              <LinkedinIcon className="w-4 h-4" />
                              Connect on LinkedIn
                            </a>
                          ) : (
                            <p className="text-xs text-slate-500 italic">This mentor hasn't added a LinkedIn URL yet.</p>
                          )}
                          {req.calendlyUrl && (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white font-bold text-xs transition-all"
                            >
                    ...
                    {isRejected && (
                      <div className="rounded-xl bg-rose-500/5 border border-rose-500/10 p-4 space-y-2">
                        ...
                        <p className="text-xs text-slate-400 leading-relaxed">
                          Unfortunately, <span className="font-semibold text-white">{req.mentorName}</span> wasn't able to take on your request at this time. Don't be discouraged — browse other mentors in the directory and send them a request.
                        </p>
                        <Link href="/dashboard/mentorship" className="inline-flex items-center gap-1.5 text-xs text-violet-400 hover:text-violet-300 font-semibold transition-colors">
